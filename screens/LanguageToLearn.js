...
const LanguageToLearn = ({navigation}) => {
  ...
  const [isLoading,setLoading] = useState(true);
    ...
    const LangIcon = ({ item, onPress, style }) => (
      ...
      useEffect(() => {
        fetch(`${global.apiUrl}/base_languages`)
        ...
        return (
          <LangIcon
            ...
            onPress={() => {
              ...
              const languageIdContainer = {languageId: item.id};
              navigation.navigate("LanguageLevelToLearn", languageIdContainer);
            }}
            style={{ borderColor }}
          />
        );
        ...
        {isLoading ? <ActivityIndicator size="large" color="#b5c2c6"  style={{ flex: 1, justifyContent: 'center'}}/> : ( 
                      ...
                      ListHeaderComponent={
                        <View style={styles.headerTextBlock}>
                          ...
                          <Text style={styles.headerTitle}>{I18n.t("Courses")}</Text>
                            <MaterialIcons 
                            name="navigate-before"
                            color="transparent"
                            size={34}
